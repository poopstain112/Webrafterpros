...
Return ONLY the complete HTML with embedded CSS - no explanations, no markdown blocks.`;
  ...
  try {
    ...
    customWebsiteHtml = aiResponse.choices[0].message.content || '';
    ...
    // Use the existing template as fallback
    customWebsiteHtml = generateBasicWebsite(businessDetails, imageUrls);
  }

  return customWebsiteHtml;

  function generateBasicWebsite(details: any, imgs: string[]) {
    const businessContent = {
      heroTitle: details.businessName || "Your Business",
      heroTagline: details.valueProposition || "Professional Services",
      services: [
        { title: details.services || "Primary Service", description: `Professional ${details.services?.toLowerCase() || 'service'} tailored to your needs` },
        { title: "Consultation", description: `Expert consultation services` },
        { title: "Premium Support", description: `Comprehensive support for all clients` }
      ],
      aboutText: `${details.businessName || 'Our business'} ${details.businessDescription || 'provides professional services'}. Located in ${details.location || 'your area'}, we serve ${details.targetAudience || 'clients'} with ${details.valueProposition || 'exceptional service'}.`,
      ctaText: details.callToAction || "Get Started Today"
    };
